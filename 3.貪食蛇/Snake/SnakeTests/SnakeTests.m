...
{
    STAssertTrue([self isEqualPoint1:p1 andPoint2:p2], @"測試點座標必需為 (%d, %d)", p2.x, p2.y);
}
...
- (void)testHitSnakeBody
{
    ...
    STAssertFalse([snakeModel isHeadHitSnakeBody], @"尚未撞到身體");
    
    [snakeModel changeMoveDirection:directionRight];
    [snakeModel moveSnake];

    STAssertTrue([snakeModel isHeadHitSnakeBody], @"已撞到身體");
}
...
- (void)testEatFruit
{
    ...
    STAssertEquals(snakeModel.snakePoints.count, (NSUInteger)3, @"吃到水果後，蛇長度要增加至 3");
